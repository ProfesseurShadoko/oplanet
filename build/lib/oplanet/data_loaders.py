...
def check_if_old(source: Literal["eu", "nasa"], max_age_days: int = 50):
    ...
    if archive_date is None:
        Message(f"No archive file found for source {source}. Please download it by running `python -m oplanet.setup`.", "!")
    elif (datetime.datetime.now() - archive_date).days > max_age_days:
        Message(f"Archive file is older than {max_age_days} days. Please refresh it by running `python -m oplanet.setup`.", "!")
